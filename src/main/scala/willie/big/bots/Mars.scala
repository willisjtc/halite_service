...
class Mars(myId: Int) extends Bot {
  ...
  private def scoreMySites(grid: Grid) = {
    ...
    scoreMySitesRec(grid, grid.borderSites, grid.borderSites, 1)
  }
  ...
  private def scoreMySitesRec(grid: Grid, borderingSites: Set[Site], totalScoredSites: Set[Site], depth: Int): Iterable[Move] = {
    ...
    if (newBorderSites.isEmpty) {
      ...
      scoreMySitesRec(grid, newBorderSites, totalScoredSites ++ newBorderSites, depth + 4)
    }
  }
  ...
  private def directions(grid: Grid, scoredSites: Set[Site]) = {
    scoredSites.filter(site => site.id == myId)
               .map(site => {
                 ...
                 if (neighbor.site.id != myId && site.strength < neighbor.site.strength || site.strength <= 6 * site.production)
